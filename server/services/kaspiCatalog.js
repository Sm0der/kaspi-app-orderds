const axios = require('axios');

// Официальный API продавца (kaspiService.js) отдаёт картинку только в одну сторону -
// когда сам продавец ЗАГРУЖАЕТ товар (поле images:[{url}] в фиде). Прочитать картинку по
// артикулу уже существующего товара через него нельзя. У Kaspi есть публичный поиск (тот
// же, что видит покупатель на сайте) - по точному артикулу он почти всегда возвращает
// ровно одну карточку с готовыми ссылками на CDN, и токен ему не нужен.
//
// ВАЖНО: этот поиск нельзя дёргать из самого приложения (Vercel) - Kaspi блокирует его
// по IP: 30 из 30 запросов подряд получили 429 с HTML-страницей защиты от ботов вместо
// JSON, с первой же попытки, до всякого разбора конкретных артикулов (проверено логами
// прямо с продакшена). Официальный API заказов при этом не блокируется - это разные
// системы защиты, публичный поиск охраняется агрессивнее как раз против таких скриптов.
// С обычного (не дата-центрового) адреса тот же запрос отрабатывает нормально, поэтому
// пополнение каталога картинками делается разово вручную (см. PRODUCT_IMAGES.md), а не
// через кнопку в интерфейсе.
const SEARCH_URL = 'https://kaspi.kz/yml/product-view/pl/filters';
const MAX_RETRIES = 2;
const RETRY_DELAY = 800;

const client = axios.create({
  timeout: 15000,
  headers: {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0 Safari/537.36',
    'Accept': 'application/json, text/plain, */*',
    'Referer': 'https://kaspi.kz/shop/search/'
  }
});

// Найти картинку товара по артикулу (коду оффера) через публичный поиск Kaspi.
// Возвращает null, если товар не нашёлся или совпадение неточное - лучше остаться
// без картинки, чем случайно подставить чужую по похожему запросу (поиск нечёткий).
async function findImageBySku(sku) {
  let attempt = 0;

  while (attempt < MAX_RETRIES) {
    try {
      const { data } = await client.get(SEARCH_URL, {
        params: { text: sku, page: 0, all: false, fl: true, ui: 'd' }
      });

      const cards = data?.data?.cards || [];
      const exact = cards.find(c => String(c.configSku) === String(sku) || String(c.id) === String(sku));
      if (!exact) return null;

      const image = exact.previewImages?.[0];
      return image ? { imageUrl: image.medium || image.large || image.small, title: exact.title } : null;
    } catch (error) {
      attempt++;
      const status = error.response?.status;
      const retriable = !error.response || status === 429 || status >= 500;
      if (!retriable || attempt >= MAX_RETRIES) {
        throw new Error(status ? `Kaspi ответил ${status}` : error.message);
      }
      await new Promise(r => setTimeout(r, RETRY_DELAY * attempt));
    }
  }
}

module.exports = { findImageBySku };
